...
class LstmImpl(torch.nn.Module):
    def __init__(self):
        super().__init__()

        print("--------------------------------------------------")
        print("LOADING RNNT LSTM POST: ORIGINAL PYTORCH QUANTIZED")
        print("--------------------------------------------------")
...
# This has to be outside the class to stop load_state_dictionary trying to load the weights
lstm = torch.quantization.quantize_dynamic(LstmImpl(), {torch.nn.LSTM}, dtype=torch.qint8)
...
class PluginLstmRnntPost(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, 
        ...
        return lstm.forward(x, init_states)
