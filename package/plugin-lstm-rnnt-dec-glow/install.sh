...
echo ""
echo "Installing GLOW LSTM RNNT Dec plugin ..."
echo ""
...
${CK_ENV_COMPILER_GLOW}/bin/model-compiler -backend=CPU -model=${CK_NNTEST_PLUGIN_DEC_ONNX}/plugin/model.onnx -emit-bundle=${BUNDLE_DIR} -relocation-model=pic -bundle-api-verbose -onnx-define-symbol=batch,1
