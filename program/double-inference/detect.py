...
params["FROZEN_GRAPH_2"] = os.getenv("CK_ALT_ENV_TENSORFLOW_MODEL_FROZEN_GRAPH")
...
def create_session(model_path):
  print("creating session with", model_path)
  g = tf.Graph()
  with g.as_default():
...
#  print('custom model is: ',CUSTOM_MODEL)
  ...
  images_processed = 0
  processed_image_ids = []
  loop_limit = len(image_files)# if (params["ENABLE_BATCH"]) == 0 else params["BATCH_COUNT"]  #defines loop boundary, that is different if batch or non batch processing are involved. the structure of the loop is however the same.i
...
#    current_session = iter_num%2
    ...
    image_data,processed_image_ids,image_size,original_image = func_defs[current_session]["preprocess"](image_files,iter_num,processed_image_ids,params)
...
##    output_dict = sessions[current_session].run(tensor_dict[current_session], feed_dict)
    ...
    out_classes = func_defs[current_session]["postprocess"](image_files,iter_num, image_size,original_image,image_data,output_dict, category_index, params)
...
'''
#is_custom = array of two binary
def init_sessions( is_custom ):
  defs = []
  for val in is_custom:
    func_defs={}
    if val == 0 :
      ##non custom
      ...
      func_defs["load_graph"]  = create_session
      ...
      func_defs["load_graph"]  = create_session
      func_defs["out_conv"]    = no_conv
    defs.append(func_defs)
  return defs

## handles custom model for both inference graphs.
def init(params):
  defs = []
  if params["CUSTOM_MODEL"] == 0:
    defs = init_sessions([0,0])
  elif params["CUSTOM_MODEL"] == 1:
    defs = init_sessions([0,1])
  elif params["CUSTOM_MODEL"] == 2:
    defs = init_sessions([1,0])
  elif params["CUSTOM_MODEL"] == 3:
    defs = init_sessions([1,1])
