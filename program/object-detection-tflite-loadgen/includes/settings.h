...
public:
    ...
    const int get_verbosity_level() const { return _verbose; }

    const std::vector<FileInfo> &image_list() const { return _image_list; }
    const std::vector<FileInfo> &list_of_available_imagefiles() const { return _image_list; }
