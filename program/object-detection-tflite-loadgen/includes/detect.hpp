...
class TFLiteBenchmark : public Benchmark<TInData, TOutData, TInConverter, TOutConverter> {
public:
    TFLiteBenchmark(BenchmarkSettings *settings, Interpreter *interpreter, int input_index)
            : Benchmark<TInData, TOutData, TInConverter, TOutConverter>(
            settings, interpreter->typed_tensor<TInData>(input_index),
            interpreter->typed_output_tensor<TOutData>(0),
            interpreter->typed_output_tensor<TOutData>(1),
            interpreter->typed_output_tensor<TOutData>(2),
            interpreter->typed_output_tensor<TOutData>(3)) {
    }
};
