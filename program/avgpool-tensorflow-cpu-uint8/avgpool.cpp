...
int main() {
  ...
  quint8 *in_data = get_random_raw_data<quint8>(in_shape, 0, 255);
  print_input_raw_data(in_data, in_shape);
  convert_data_layout_NCHW_to_NHWC(in_data, in_shape);
  Tensor input(DT_QUINT8, get_tensor_shape(in_shape));
  ...
  QuantizedAvgPool avgpool(root.WithOpName(test_node_name),
                           input,
                           in_data_min, in_data_max,
  ...
  measure_test([&]() {
    ...
    auto result = session.Run(run_options, {}, {avgpool.output}, {}, &outputs, &run_metadata);
    TF_CHECK_OK(result);
  });
