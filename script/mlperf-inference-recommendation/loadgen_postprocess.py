#
# MLPerf inference; recommendation; postprocessing
#
...
def ck_postprocess(i):
  ...
  if accuracy_mode:
    ...
    matchObj  = re.search(r'AUC=([\d\.]+)%,\s*accuracy=([\d\.]+)%,\s*good=(\d+),\s*total=(\d+),\s*queries=(\d+)', output)
    save_dict['AUC']      = float( matchObj.group(1) )
    save_dict['accuracy'] = float( matchObj.group(2) )
    save_dict['good']     = int( matchObj.group(3) )
    save_dict['total']    = int( matchObj.group(4) )
    save_dict['queries']  = int( matchObj.group(5) )
